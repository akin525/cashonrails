...
const Notifications: React.FC = () => {
    ...
    // Filter notifications based on the active tab and date range
    const filteredNotifications = notifications.filter(notification => {
        const isInDateRange = (!startDate || notification.date >= startDate) && (!endDate || notification.date <= endDate);
        if (activeTab === 'all') return isInDateRange;
        return notification.type === activeTab.toLowerCase() && isInDateRange; // Convert activeTab to lowercase for comparison
    });
    ...
    return (
        <div className="w-full max-w-5xl mt-10 p-5 bg-white">
            <header className="flex justify-between items-center pb-3 mb-3">
                <h2 className="text-xl font-semibold text-gray-900">Notifications</h2>
            </header>

            <div className="flex space-x-4 mb-3 justify-between items-center gap-2 flex-wrap">
               ...
               {['all', 'transactions', 'transfers', 'subscriptions'].map((tab) => (
                    ...
                    >
                        {tab} 
                        <span className={`text-xs ${activeTab === tab ? 'bg-black text-gray-200' : 'bg-gray-200 text-gray-600'} px-1 rounded ml-1`}>
                 ...
                 <DateRangeSelector 
                    ...
                    onRelativeRangeChange={handleDateRangeChange} 
