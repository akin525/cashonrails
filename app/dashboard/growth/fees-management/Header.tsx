...
import CustomFilter, { BaseFilterConfig, FilterState } from '@/components/dashboard/filters/CustomFilter';

const Header = () => {
    const [isFilterOpen, setIsFilterOpen] = useState(false);
    const { setFilterState } = useUI();

    const filterConfig: BaseFilterConfig[] = [
        {
            type: 'select',
            label: 'Status',
            name: 'status',
    ...
    return (
        <header className="flex items-center justify-between p-4 border-b border-bar min-h-auto relative flex-wrap gap-2">
            ...
            <nav className="flex items-center space-x-3">
                <button className='border-[#D0D5DD] border p-3 rounded-lg' onClick={() => setIsFilterOpen(true)}>
                    <FilterIcon />
                </button>
                {/*<CustomFilter */}
                {/*    isOpen={isFilterOpen}*/}
