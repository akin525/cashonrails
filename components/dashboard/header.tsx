...
import FormInputs, { FormSelect } from '../formInputs/formInputs';
import DateRangeSelector from '../filter/DateRangePicker';
...
const Header: React.FC = () => {
  const { setSearchQuery, showSearchQuery, headerTitle, setDateFilter } = useUI();
    ...
    <>
      <header className="flex items-center justify-between p-4 border-b border-bar min-h-auto relative flex-wrap gap-2">
        ...
        <nav className="flex items-center space-x-3">
          <DateRangeSelector defaultRelativeRangeValue={'30d'} onRelativeRangeChange={handleDateFilterChange}
            startDate={new Date()} endDate={new Date()} onDateChange={handleDateFilterChange} />
